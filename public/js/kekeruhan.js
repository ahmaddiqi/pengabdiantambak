...
var indikator = 'Kekeruhan';
var id ='myAreaChartKekeruhan';
var idtabel ="tabelKekeruhan";
var min, max;
var db2 = firebase.database().ref('device/setPoint/Kekeruhan')
db2.on('value', (e)=>{
    min = (e.val().min);
    max = (e.val().max);
})
...
function fil(result){
    ...
    for(let i in result){
        ...
        min0.push(min);
        max0.push(max);
        tabel(idtabel, `${d}/${e+1}/${f} ${b}:${a}`, result[i].turbidity, min, max);
    }
